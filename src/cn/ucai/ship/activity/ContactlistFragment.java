...
import cn.ucai.ship.applib.controller.HXSDKHelper.HXSyncListener;
import com.easemob.chat.EMContactManager;
import cn.ucai.ship.Constant;
import cn.ucai.ship.DemoHXSDKHelper;
import cn.ucai.ship.adapter.ContactAdapter;
import cn.ucai.ship.db.InviteMessgeDao;
import cn.ucai.ship.db.UserDao;
import cn.ucai.ship.domain.User;
import cn.ucai.ship.widget.Sidebar;
...
public class ContactlistFragment extends Fragment {
				    ...
				    getActivity().runOnUiThread(new Runnable(){
		                ...
		                public void run() {
		                    if(success){
		                        ...
		                        String s1 = getResources().getString(cn.ucai.ship.R.string.get_failed_please_check);
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(cn.ucai.ship.R.layout.fragment_contact_list, container, false);
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		listView = (ListView) getView().findViewById(cn.ucai.ship.R.id.list);
		sidebar = (Sidebar) getView().findViewById(cn.ucai.ship.R.id.sidebar);
		...
		//搜索框
		query = (EditText) getView().findViewById(cn.ucai.ship.R.id.query);
		query.setHint(cn.ucai.ship.R.string.search);
		clearSearch = (ImageButton) getView().findViewById(cn.ucai.ship.R.id.search_clear);
		...
		// 设置adapter
		adapter = new ContactAdapter(getActivity(), cn.ucai.ship.R.layout.row_contact, contactList);
		...
		ImageView addContactView = (ImageView) getView().findViewById(cn.ucai.ship.R.id.iv_new_contact);
		// 进入添加好友页
		...
		progressBar = (View) getView().findViewById(cn.ucai.ship.R.id.progress_bar);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		if (((AdapterContextMenuInfo) menuInfo).position > 3) {
			...
			getActivity().getMenuInflater().inflate(cn.ucai.ship.R.menu.context_contact_list, menu);
		}
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == cn.ucai.ship.R.id.delete_contact) {
			try {
                // 删除此联系人
			...
			return true;
		}else if(item.getItemId() == cn.ucai.ship.R.id.add_to_blacklist){
	...
	public void deleteContact(final User tobeDeleteUser) {
		String st1 = getResources().getString(cn.ucai.ship.R.string.deleting);
		final String st2 = getResources().getString(cn.ucai.ship.R.string.Delete_failed);
	...
	private void moveToBlacklist(final String username){
		final ProgressDialog pd = new ProgressDialog(getActivity());
		String st1 = getResources().getString(cn.ucai.ship.R.string.Is_moved_into_blacklist);
		final String st2 = getResources().getString(cn.ucai.ship.R.string.Move_into_blacklist_success);
		final String st3 = getResources().getString(cn.ucai.ship.R.string.Move_into_blacklist_failure);
		pd.setMessage(st1);
		pd.setCanceledOnTouchOutside(false);
		pd.show();
		new Thread(new Runnable() {
