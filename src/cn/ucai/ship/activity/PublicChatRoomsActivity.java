...
public class PublicChatRoomsActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(cn.ucai.ship.R.layout.activity_public_groups);

		// 搜索框
		searching = (EditText)findViewById(cn.ucai.ship.R.id.query);
		clean = (ImageButton)findViewById(cn.ucai.ship.R.id.search_clear);
		searching.setHint(cn.ucai.ship.R.string.search);
		inputMethodManager = (InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
		
		pb = (ProgressBar) findViewById(cn.ucai.ship.R.id.progressBar);
		listView = (ListView) findViewById(cn.ucai.ship.R.id.list);
		title = (TextView)findViewById(cn.ucai.ship.R.id.tv_title);
		title.setText(getResources().getString(cn.ucai.ship.R.string.chat_room));
		chatRoomList = new ArrayList<EMChatRoom>();
		rooms = new ArrayList<EMChatRoom>();
		
		View footView = getLayoutInflater().inflate(cn.ucai.ship.R.layout.listview_footer_view, null);
        footLoadingLayout = (LinearLayout) footView.findViewById(cn.ucai.ship.R.id.loading_layout);
        footLoadingPB = (ProgressBar)footView.findViewById(cn.ucai.ship.R.id.loading_bar);
        footLoadingText = (TextView) footView.findViewById(cn.ucai.ship.R.id.loading_text);
...
//                                    footLoadingLayout.setVisibility(View.VISIBLE);
                            ...
                            if(isFirstLoading){
                                ...
                                if(chatRooms.size() < pagesize){
                                    ...
                                    footLoadingText.setText(getResources().getString(cn.ucai.ship.R.string.no_more_messages));
                                }
                                adapter.notifyDataSetChanged();
                            }
                            isLoading = false;
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            pb.setVisibility(View.INVISIBLE);
                            footLoadingLayout.setVisibility(View.GONE);
                            Toast.makeText(PublicChatRoomsActivity.this, getResources().getString(cn.ucai.ship.R.string.failed_to_load_data), 0).show();
                        }
                    });
                }
            }
	...
	private class ChatRoomAdapter extends ArrayAdapter<EMChatRoom> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			((TextView) convertView.findViewById(cn.ucai.ship.R.id.name)).setText(getItem(position).getName());
